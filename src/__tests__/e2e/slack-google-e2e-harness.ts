...
function getSlackPostToken(): string {
  ...
  if (!_warnedPostingAsBot) {
    ...
    console.warn(
      "[Slack E2E] Posting as bot. To have test messages appear as you, set TEST_SLACK_USER_TOKEN in .env.test.local or surfaces/.env.local (user OAuth token starting with xoxp-, scope chat:write:user). See src/__tests__/e2e/README.md.",
    );
  }
