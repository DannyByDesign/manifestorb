...
- **Run:** `RUN_LIVE_E2E=true bunx vitest run --config vitest.e2e.config.ts src/__tests__/e2e/critical-e2e-*.test.ts`

## Slack ↔ Main App ↔ Google E2E

Three-way integration tests: Slack (Sidecar) ↔ Main App (agent) ↔ Google Suite (Gmail/Calendar).

### Env

- **Slack:** `SLACK_BOT_TOKEN`, `TEST_SLACK_CHANNEL_ID`, `TEST_SLACK_USER_ID`. Can live in `surfaces/.env.local`; when running from the main app, copy or symlink into `.env.test.local` (or load surfaces env in setup; e2e-setup loads `surfaces/.env.local` when `RUN_LIVE_SLACK_GOOGLE_E2E=true`).
- **Slack (messages as you):** Optional `TEST_SLACK_USER_TOKEN` — a **user** OAuth token (starts with `xoxp-`, **not** the bot token `xoxb-`) with scope `chat:write:user`. When set, messages posted by the tests appear as you instead of the bot. To get it: Slack app → OAuth & Permissions → add User Token Scopes `chat:write:user` (and `chat:write` if needed) → Reinstall to workspace → copy the **OAuth Access Token** (the `xoxp-...` one) into `.env.test.local` or `surfaces/.env.local` as `TEST_SLACK_USER_TOKEN`. If you see "[Slack E2E] Posting as bot" in test output, this var is missing or wrong.
...
- **Tier 1 only (simulated round-trip):** Main app running; no sidecar or real Slack required for the simulated test.
  ```bash
  RUN_LIVE_SLACK_GOOGLE_E2E=true SURFACES_SHARED_SECRET=your-secret bunx vitest run --config vitest.e2e.config.ts src/__tests__/e2e/critical-e2e-slack-google-tier1-basic.test.ts
  ```
- **Full three-way (all tiers):** Main app (e.g. port 3000) and surfaces sidecar (e.g. port 3001) running; Slack env set. Test files run **serially** (fileParallelism: false) and each test waits **15s** before starting, and each Slack message post waits **15s** after sending (in the harness) so messages are spaced and the bot isn’t flooded. The bot replies in the **channel** (not in threads). User follow-ups are sent as **top-level channel messages** (no `thread_ts`), and tests wait for the bot via `waitForSlackChannelResponse(channel, { afterTs })` (not thread replies). For multi-turn tests, send each follow-up **after** the AI responds so the bot isn’t given multiple messages in a row. so the bot isn't flooded; messages stay ordered and the model isn't confused by multiple concurrent requests.
  ```bash
  RUN_LIVE_SLACK_GOOGLE_E2E=true RUN_LIVE_E2E=true bunx vitest run --config vitest.e2e.config.ts src/__tests__/e2e/critical-e2e-slack-google-*.test.ts
  ```
