...
import { env } from "@/env";
import { ssoRegistrationBody } from "@/actions/sso.validation";
import { adminActionClient } from "@/actions/safe-action";
import { auth } from "@/server/auth";
import { SafeError } from "@/server/lib/error";
import { extractSSOProviderConfigFromXML } from "@/server/lib/sso/extract-sso-provider-config-from-xml";
import prisma from "@/server/db/client";
import { validateIdpMetadata } from "@/server/lib/sso/validate-idp-metadata";
import { slugify } from "@/server/lib/string";
