...
vi.mock("@/server/lib/redis/account-validation");

// Mock specific functions from @/server/lib/error, keep original SafeError
vi.mock("@/server/lib/error", async (importActual) => {
  const actual = await importActual<typeof import("@/server/lib/error")>();
  return {
