...
vi.mock("@/features/rules/ai/match-rules", () => ({
...
vi.mock("@/features/rules/ai/choose-args", () => ({
  getActionItemsWithAiArgs: vi.fn(),
}));
vi.mock("@/features/rules/ai/execute", () => ({
  executeAct: vi.fn(),
}));
vi.mock("@/features/reply-tracker/label-helpers", () => ({
...
vi.mock("@/features/scheduled/scheduler", () => ({
...
describe("runRules - double draft prevention", () => {
  ...
  it("executes only one DRAFT_EMAIL when custom rule and TO_REPLY both have drafts", async () => {
    const { findMatchingRules } = await import(
      "@/features/rules/ai/match-rules"
    );
    const { determineConversationStatus } = await import(
      "@/features/reply-tracker/handle-conversation-status"
    );
    const { getActionItemsWithAiArgs } = await import(
      "@/features/rules/ai/choose-args"
    );
    const { executeAct } = await import("@/features/rules/ai/execute");
