...
import { updateThreadTrackers } from "@/server/utils/reply-tracker/handle-conversation-status";
        ...
        switch (resource) {
            case "email":
                ...
                if (changes.followUp) {
                    ...
                    const { applyFollowUpLabel, removeFollowUpLabel } = await import("@/server/utils/follow-up/labels");
                    const emails = await providers.email.get(ids);
                    if (emails.length === 0) return { success: false, error: "Emails not found" };
