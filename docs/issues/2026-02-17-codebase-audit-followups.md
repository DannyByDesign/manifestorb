...
## Follow-up items
1. [open] Remove disconnected legacy memory toolpack module (`src/server/features/ai/memory-tools.ts`) and update docs that still reference it (`src/server/features/memory/ARCHITECTURE.md`, `src/server/features/memory/embeddings/README.md`).
...
3. [open] Define lint strategy and baseline cleanup plan. Current global lint gate fails with large pre-existing debt (hundreds of `no-explicit-any` and other violations), so CI signal is noisy for incremental runtime/search work.
