...
2) Staging smoke tests pass for Google MVP flows (Gmail draft/send approvals, Calendar scheduling + conflicts, notifications fallback).
3) Webhook/cron verification complete for Gmail/Calendar watch renewals and scheduled-actions execute (QStash signature + CRON_SECRET).
